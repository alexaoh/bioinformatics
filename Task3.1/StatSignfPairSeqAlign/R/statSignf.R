...
statSignf <- function(seq1, seq2, kind.seq = "Protein", kind.align = "global", subst.matrix = "BLOSUM62",
  ...
  # Load the dependencies inside function for now. Should not be done like this.
  #library(seqinr)
  #library(Biostrings)
  options(warn=-1)
  data(list = subst.matrix) # This works, but shows warning (think it because of the imports when making the package).
  options(warn=0)
  ...
  if (kind.align == "global"){
    if (kind.seq == "Protein"){
      ...
      OG.score <- pairwiseAlignment(s1, s2, gapOpening = gap.scores[1],
                                    gapExtension = gap.scores[2], scoreOnly = TRUE, type = "global")
    }
  } else {
    if (kind.seq == "Protein"){
      OG.score <- pairwiseAlignment(s1, s2, substitutionMatrix = subst.matrix,
      ...
      OG.score <- pairwiseAlignment(s1, s2,
                                    gapOpening = gap.scores[1], gapExtension = gap.scores[2],
                                    scoreOnly = TRUE, type = "local")
    }
  }
  ...
  if (suppress.output == FALSE){
    ...
    cat("The estimations of the parameters of the Gumbel distribution are: \n Scale parameter lambda = ", lambda.hat,
        " and mode u = ", u.hat, ". \n", sep = "")
    ...
    cat(format(c("Min.", "1st Qu.", "Median", "Mean", "3rd Qu.", "Max"), width = 8, justify = "left"))
    cat("\n")
    cat(format(c(score.min, score.1st.quant, score.median, score.mean, score.3rd.quant, score.max),
